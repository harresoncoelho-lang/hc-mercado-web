...
5. **`data/oportunidades_meta.json` não ficou git-tracked como as Tasks 2/3/4/6 abaixo descrevem — o metadado foi pro Supabase também**, como um blob em `dados_robo`/`"oportunidades_meta"` (mesmo padrão de `convenios`/`sistema_s_am`, ver `carregarBlobSupabase`/`salvarBlob`). Motivo: manter `oportunidades_meta.json` comitado no git é incompatível com a Task 5 (parar de comitar no workflow de recuperação horária) — sem commit, o `coberturaPorUf` acumulado morreria a cada execução do job de recuperação, e `ufsPendentesDeAtualizacao` (que depende desse metadado sobreviver entre execuções) voltaria a reprocessar as mesmas UFs a cada hora. Guardá-lo no Supabase (que já existe pra este exato problema, dos irmãos `convenios`/`sistema_s_am`) resolve sem reintroduzir commits automáticos. `painel.html` lê esse blob com `carregarBlobSupabase("oportunidades_meta")` em vez de `carregarMeta("data/oportunidades_meta.json")`; o shape do metadado (`atualizadoEm`, `coberturaPorUf`, etc.) é o mesmo descrito nas tarefas abaixo.
