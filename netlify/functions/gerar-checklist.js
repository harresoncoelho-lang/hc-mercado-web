...
function util(valor) {
  ...
  return resultado && !/^n[ãa]o (informado|localizado)/i.test(resultado) ? resultado : "";
}
...
function paragrafo(conteudo, { negrito = false, tamanho = 21, cor = "17243A", antes = 0, depois = 120, alinhamento = "left" } = {}) {
  ...
  const propriedades = `<w:rPr><w:color w:val="${cor}"/><w:sz w:val="${tamanho}"/>${negrito ? '<w:b/>' : ""}</w:rPr>`;
...
function tituloSecao(titulo) {
  return `<w:p><w:pPr><w:spacing w:before="240" w:after="90"/><w:shd w:val="clear" w:fill="EEF3F8"/><w:pBdr><w:left w:val="single" w:sz="18" w:space="8" w:color="1F75DF"/></w:pBdr></w:pPr><w:r><w:rPr><w:b/><w:color w:val="193A63"/><w:sz w:val="22"/></w:rPr><w:t>${xml(titulo)}</w:t></w:r></w:p>`;
}
...
function conteudoDocumento(edital, resumo) {
  ...
  const dadosBasicos = tabelaDados([["Número", numero], ["Órgão", orgao.nome || edital.orgao], ["Modalidade", identificacao.modalidade || edital.modalidade], ["Data e horário da sessão", [util(sessao.data), util(sessao.horario)].filter(Boolean).join(" às ")], ["Prazo final de propostas", edital.encerramento || resumo.prazos?.limiteEnvioPropostas], ["Critério de julgamento", detalhes.criterioJulgamento], ["Regime de execução", detalhes.regimeExecucao || edital.regimeExecucao]]);
  const condicoes = tabelaDados([["Prazo de entrega", detalhes.prazoEntrega], ["Validade da proposta", resumo.criteriosProposta?.validadeProposta], ["Condições de pagamento", resumo.condicoesPagamento], ["Garantia de proposta", resumo.garantias?.proposta], ["Garantia contratual", resumo.garantias?.contrato], ["Legislação e base legal", resumo.legislacao]]);
  ...
  return `${cabecalho}${paragrafo("Checklist de Licitação", { negrito: true, tamanho: 36, cor: "102D56", antes: 280, depois: 45 })}${paragrafo(`Processo ${numero}`, { tamanho: 20, cor: "637085", depois: 180 })}${destaque}${tituloSecao("Identificação e prazos")}${dadosBasicos}${listaComFallback("Documentos de habilitação", documentos, "Nenhum documento específico foi identificado no material analisado. Confira o edital oficial.")}${tituloSecao("Condições comerciais e operacionais")}${condicoes}${listaComFallback("Declarações e formulários", declaracoes, "Nenhuma declaração específica foi identificada no material analisado.")}${listaComFallback("Pontos para conferir antes da proposta", pendencias, "Revise o edital, os anexos e o termo de referência antes de apresentar a proposta.")}${tituloSecao("Síntese do edital")}${paragrafo(util(resumo.resumoGeral) || "O resumo completo não está disponível. Consulte o edital e seus anexos oficiais.", { tamanho: 20, depois: 180 })}${paragrafo("Documento gerado pelo LicitaPlena como apoio operacional. Confirme sempre o edital e os anexos oficiais antes de decidir.", { tamanho: 16, cor: "637085", antes: 180, depois: 0 })}`;
}
...
function montarDocx(edital, resumo) {
  ...
  zip.addFile("word/styles.xml", Buffer.from(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><w:styles xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:docDefaults><w:rPrDefault><w:rPr><w:rFonts w:ascii="Aptos" w:hAnsi="Aptos"/><w:sz w:val="20"/></w:rPr></w:rPrDefault></w:docDefaults></w:styles>`));
...
exports.__test = { montarDocx, conteudoDocumento, util };
